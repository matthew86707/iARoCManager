 ...
 */
package com.jointheleague.iaroc.ejb;

import com.jointheleague.iaroc.entities.Team;
...
public class TeamRequestBean {
    ...
    public void createTeam(String teamId, String name, String imageUrl) {
        try {
            Team team = new Team(teamId, name, imageUrl);
            logger.log(Level.INFO, "Created team {0}", teamId);
            em.persist(team);
            logger.log(Level.INFO, "Persisted team {0}", teamId);
        } catch (Exception ex) {
